...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], checkpoint_ply=None):
        """b
        ...
        if os.path.exists(os.path.join(args.source_path, "intri.yml")):
            scene_info = sceneLoadTypeCallbacks["Easymocap"](args.source_path, args.white_background, args.eval)
        ...
        if not self.loaded_iter:
            ...
            if isinstance(camlist[0], Camera):
        ...
        if self.loaded_iter:
            checkpoint_path = os.path.join(self.model_path, "point_cloud", "iteration_" + str(self.loaded_iter), "point_cloud.ply")
