...
python3 train.py --eval --white_background --SSAA 1 --resolution 1 --expname "$expname" --meshscale 2.0 --start_checkpoint_ply "$checkpoint" -s "$data_path" || { echo "Second train.py failed"; exit 1; }
...
python metric.py --gt_path "$gt_path" --pred_path "$pred_path" --mode "body" || { echo "metric.py failed"; exit 1; }
