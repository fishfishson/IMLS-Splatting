...
# from utils.parallel_utils import parallel_execution
...
def load_image_from_bytes(buffer: np.ndarray, ratio=1.0, normalize=False, decode_flag=cv2.IMREAD_UNCHANGED):
    ...
    if image.ndim == 2:
        image = image[..., None]  # NOTE: THIS MUST HAPPEN AFTER RESIZE TO AVOID DIMENSION MISMATCH FOR CV2
    return image
...
def decode_crop_fill_im_bytes(im_bytes: BytesIO,
    ...
    # prevent masked zoomed in view to be empty
    # mh = max(1, mh)
    # mw = max(1, mw)
    ...
    # img = (img * (msk > 255*0.5)).clip(0, 255).astype(np.uint8)  # fill black with hard mask
